    ...
    public static ArrayList<String> check(ArrayList<String> list) throws MyException {
        ...
        try {
            if (map.containsValue(Integer.parseInt(list.get(0))) & map.containsValue(Integer.parseInt(list.get(1)))) {
                ...
                if (Integer.parseInt(list.get(0)) < 1 || Integer.parseInt(list.get(0)) > 10 || Integer.parseInt(list.get(1)) < 1 || Integer.parseInt(list.get(1)) > 10) {
                    throw new MyException("Одно или оба числа не входят в диапозон [1-10]");
                }
            }
        } catch (NumberFormatException e) {
            throw new MyException("Неверный ввод чисел");
        }
        return list;
    }
}
